...
def configure_trigger(cam):
    """
    This function configures the camera to use a hardware trigger. First, trigger mode is
    ...
    try:
        ...
        # Select trigger source to hardware (Line0)
        ...
        if not PySpin.IsReadable(node_trigger_overlap_readout):
            print('Unable to set trigger overlap (enum entry retrieval). Aborting...')
            return False
        node_trigger_overlap.SetIntValue(node_trigger_overlap_readout.GetValue())
        print('Trigger overlap set to ReadOut...')

        # Set trigger activation to RisingEdge
        node_trigger_activation = PySpin.CEnumerationPtr(nodemap.GetNode('TriggerActivation'))
        if not PySpin.IsReadable(node_trigger_activation) or not PySpin.IsWritable(node_trigger_activation):
        ...
        if not PySpin.IsReadable(node_trigger_activation_rising_edge):
...
def run_single_camera(cam):
    """
    This function acts as the body of the example. First, it retrieves the serial number of the device. 
    It then initializes the camera, calls the example functions, and cleans up afterward.
